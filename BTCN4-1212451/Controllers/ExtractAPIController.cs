...
namespace BTCN4_1212451.Controllers
{
    ...
    public class ExtractAPIController : ApiController
    {
        ...
        public async Task<IEnumerable<News>> GetNews()
        {
            ...
            var cell_taga_title = document.QuerySelectorAll(tagtd_title);
            var cell_taga_day_month = document.QuerySelectorAll(tagtd_day_month);
            var cell_taga_post_year = document.QuerySelectorAll(tagtd_post_year);
            ...
            List<String> list_cells_taga_title = cell_taga_title.Select(m => m.TextContent).ToList();
            List<String> list_cells_tagtd_day_month = cell_taga_day_month.Select(m => m.TextContent).ToList();
            List<String> list_cells_tagtd_post_year = cell_taga_post_year.Select(m => m.TextContent).ToList();

            //int amout = list_cells_tagaShowPostLinks.Count();
            int amout = list_cells_taga_title.Count();
            Regex number = new Regex(@"\d+");
            for (int i = 0; i < amout; i++)
            {
                string str_title = list_cells_taga_title[i].Replace('\n', '\t');
